 ...
 * Core Game Logic (Final Robust Version)
 */
...
let isBaptisteImgLoaded = false;
baptisteImg.onload = () => {
    isBaptisteImgLoaded = true;
    console.log('Baptiste image loaded');
};
baptisteImg.onerror = () => {
    console.error('Failed to load image at:', baptisteImg.src);
};
...
class Game {
    ...
    displayRanking() {
        ...
        leaderboardList.innerHTML = rankings.map((r, i) => `
            <li><span class="name">${i + 1}. ${r.name}</span><span class="score">${r.score}</span></li>
    ...
    useRegenBurst() {
        ...
        this.startCooldown(regenCooldownEl, REGEN_COOLDOWN_MS, () => { this.isRegenReady = true; });
        setTimeout(() => { this.player.isRegenActive = false; }, REGEN_DURATION_MS);
    }

    useImmortalityField() {
        if (!this.player.isAlive || !this.isImmortalityReady) return;
        this.isImmortalityReady = false;
        this.immortalityFields.push({ x: this.player.x, y: this.player.y, radius: 120, endTime: Date.now() + IMMORTALITY_DURATION_MS, alpha: 0.8 });
        this.startCooldown(immortalityCooldownEl, IMMORTALITY_COOLDOWN_MS, () => { this.isImmortalityReady = true; });
    }
    ...
    createClickEffect(x, y) { this.clickEffects.push({ x, y, radius: 5, alpha: 1 }); }

    createExplosion(x, y, color, count) {
        for (let i = 0; i < count; i++) {
            this.particles.push({ x, y, vx: (Math.random() - 0.5) * 8, vy: (Math.random() - 0.5) * 8, radius: Math.random() * 3 + 1, alpha: 1, color: color });
        }
    }
    ...
    spawnProjectile() {
        ...
        this.projectiles.push({ x, y, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed, radius: 10, color: '#ff4655', trail: [] });
    }
    ...
    update() {
        ...
        this.player.isImmortal = this.immortalityFields.some(f => Math.sqrt((f.x - this.player.x)**2 + (f.y - this.player.y)**2) < f.radius);
        ...
        for (let i = this.projectiles.length - 1; i >= 0; i--) {
            ...
            if (Math.sqrt((p.x - this.player.x)**2 + (p.y - this.player.y)**2) < p.radius + PLAYER_RADIUS - 10) {
                if (!this.player.isImmortal) this.gameOver();
            }
            if (p.x < -200 || p.x > canvas.width + 200 || p.y < -200 || p.y > canvas.height + 200) this.projectiles.splice(i, 1);
        }
        for (let i = this.immortalityFields.length - 1; i >= 0; i--) {
            const f = this.immortalityFields[i];
            if (now > f.endTime) { f.alpha -= 0.05; if (f.alpha <= 0) this.immortalityFields.splice(i, 1); }
        }
        this.particles.forEach((p, i) => { p.x += p.vx; p.y += p.vy; p.alpha -= 0.02; if (p.alpha <= 0) this.particles.splice(i, 1); });
        this.clickEffects.forEach((ef, i) => { ef.radius += 1.5; ef.alpha -= 0.05; if (ef.alpha <= 0) this.clickEffects.splice(i, 1); });
    }
    ...
    draw() {
        ...
        this.immortalityFields.forEach(f => {
            ...
            ctx.strokeStyle = `rgba(0, 212, 255, ${f.alpha})`; ctx.lineWidth = 4; ctx.stroke();
            ctx.strokeStyle = `rgba(0, 212, 255, ${f.alpha * 0.2})`; ctx.lineWidth = 1;
            ...
            ctx.fillStyle = `rgba(0, 212, 255, ${f.alpha * 0.1})`; ctx.fill();
            ctx.save(); ctx.translate(f.x, f.y); ctx.rotate(Date.now() / 200); ctx.fillStyle = '#fff'; ctx.fillRect(-8, -2, 16, 4); ctx.fillRect(-2, -8, 4, 16); ctx.restore();
        });
        this.projectiles.forEach(p => {
            ctx.beginPath(); ctx.moveTo(p.trail[0]?.x, p.trail[0]?.y); p.trail.forEach(t => ctx.lineTo(t.x, t.y));
            ctx.strokeStyle = p.color; ctx.lineWidth = 3; ctx.globalAlpha = 0.3; ctx.stroke(); ctx.globalAlpha = 1;
            ctx.beginPath(); ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2); ctx.fillStyle = p.color; ctx.shadowBlur = 15; ctx.shadowColor = p.color; ctx.fill(); ctx.shadowBlur = 0;
        });
        this.clickEffects.forEach(ef => { ctx.beginPath(); ctx.arc(ef.x, ef.y, ef.radius, 0, Math.PI * 2); ctx.strokeStyle = `rgba(0, 255, 100, ${ef.alpha})`; ctx.lineWidth = 2; ctx.stroke(); });

        // Player drawing
        const now = Date.now();
        const bobbing = Math.sin(now / 150) * 8;
        const scaleX = 1 + Math.sin(now / 100) * 0.1;
        ...
        if (this.player.isAlive) {
            const dx = this.player.targetX - this.player.x;
            const dy = this.player.targetY - this.player.y;
            if (Math.sqrt(dx*dx + dy*dy) > 5) ctx.rotate(Math.atan2(dy, dx) + Math.PI/2);
        }
        if (isBaptisteImgLoaded) {
            ctx.drawImage(baptisteImg, -30, -30, 60, 60);
        } else {
            ctx.beginPath(); ctx.arc(0, 0, PLAYER_RADIUS, 0, Math.PI * 2); ctx.fillStyle = '#00ff00'; ctx.fill();
        }
        ctx.restore();
        ctx.fillStyle = "#fff"; ctx.font = "bold 16px 'Inter', sans-serif"; ctx.textAlign = "center"; ctx.shadowBlur = 4; ctx.shadowColor = "#000"; ctx.fillText("ぱかす３１", this.player.x, this.player.y + bobbing - PLAYER_RADIUS - 25); ctx.shadowBlur = 0;
        if (this.player.isRegenActive) { ctx.beginPath(); ctx.arc(this.player.x, this.player.y, PLAYER_RADIUS + 5, 0, Math.PI * 2); ctx.strokeStyle = '#ff9d00'; ctx.lineWidth = 4; ctx.stroke(); }
        this.particles.forEach(p => { ctx.beginPath(); ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2); ctx.fillStyle = p.color; ctx.globalAlpha = p.alpha; ctx.fill(); ctx.globalAlpha = 1; });
    }
